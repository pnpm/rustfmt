...
fn field_receivers() {
    ...
    let _ = packages.iter().map(|package| package.name()).collect::<Vec<_>>();
}
...
fn comments() {
    ...
    let _ = parsed // package metadata
        .dlx.os.iter().count();
    let _ = parsed.get() // resolved package
        .dlx.os.iter().count();
    let _ = parsed.dlx.os
        // filter by platform
        .iter().count();
    let _ = parsed.dlx /* selected platform */ .os.is_empty();
}

fn long_single_calls() {
    let _ = parsed.configuration.platform.operating_system.matches_any_of_the_supported_operating_systems(operating_system);
    let _ = parsed.configuration.platform.operating_system.matches_any_of_the_supported_operating_systems(first, second, third, fourth);
    let _ = parsed.configuration.platform.operating_system.with_platform_configuration(|configuration| {
...
fn long_field_prefixes() {
    let _ = packages.configuration.platform.operating_system.supported_platforms.supported_operating_systems;
    let _ = packages.configuration.platform.operating_system.supported_platforms.supported_operating_systems.iter().count();
    let _ = packages.configuration.platform.operating_system.supported_platforms.支持平台.操作系统.iter().count();
    let _ = packages.configuration.platform.operating_system.supported_platforms.éééééééééééééééééé.iter().count();
}
...
fn complex_arguments() {
    ...
    let _ = map.get({ let selected = key; selected }).is_some();
...
fn multiline_arguments() {
    ...
    let _ = map.get("first
second").is_some();
    let _ = map.get(r#"first
second"#).is_some();
    let _ = receiver.set(first_argument_with_long_name, second_argument_with_long_name, third_argument_with_long_name).finish();
    let _ = receiver.get().set(first_argument_with_long_name, second_argument_with_long_name, third_argument_with_long_name);
    let _ = receiver.set(first_argument_with_long_name, second_argument_with_long_name, third_argument_with_long_name);
}
...
async fn await_uses_field_layout() {
    ...
    let _ = client.fetch(request()).await? // preserve await comment
        .metadata.validate().finish();
}
