...
fn short_receivers() {
    let descriptive_binding = x.iter().map(transform_element).collect::<Vec<_>>();
    let descriptive_binding = deps.iter().map(transform_element).collect::<Vec<_>>();
    let descriptive_binding = items.iter().map(transform_element).collect::<Vec<_>>();
    let descriptive_binding = values.iter().map(transform_element).collect::<Vec<_>>();
    let descriptive_binding = self.iter().map(transform_element).collect::<Vec<_>>();
    let descriptive_binding = x.a.iter().map(transform_element).collect::<Vec<_>>();
    let descriptive_binding = self.a.iter().map(transform_element).collect::<Vec<_>>();
    let descriptive_binding = get().iter().map(transform_element).collect::<Vec<_>>();
    let descriptive_binding = ééééé.iter().map(transform_element).collect::<Vec<_>>();
    let descriptive_binding = 中中.iter().map(transform_element).collect::<Vec<_>>();
    let descriptive_binding = 中中中.iter().map(transform_element).collect::<Vec<_>>();
    let _ = x.a().b().some_much_longer_final_method_name();
    let _ = x.map(nested_call(value)).some_much_longer_final_method_name();
    let _ = x.map(|value| { let result = process(value); result }).collect::<Vec<_>>();
    let _ = x.iter().field.await?.map(transform_element).collect::<Vec<_>>();
    let _ = x.iter().map(transform_element).field.await?.collect::<Vec<_>>();
    let _ = items /* receiver comment */ .iter().map(transform_element).collect::<Vec<_>>();
    let _ = items.iter() // method comment
