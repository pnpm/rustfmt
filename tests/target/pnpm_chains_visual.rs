...
fn long_field_prefixes() {
    let _ = packages.configuration.platform
...
fn unicode_field_prefixes() {
    let _ =
        packages.configuration.platform.支持平台.操作系统;
    let _ = packages.configuration.platform.支持平台
            .操作系统
            .is_empty();
    let _ = packages.configuration.platform.支持平台
            .操作系统
            ...
            .count();
    let _ =
        packages.configuration.platform.éééééééééééééééééé;
    let _ = packages.configuration.platform
            .éééééééééééééééééé
            .iter()
            .count();
