...
// rustfmt-struct_pattern_width: 35
...
fn patterns(command: Command, Pair { first, second }: Pair) {
    match command {
        ...
        Command::Long {
            first_descriptive_field_name,
            second_descriptive_field_name,
        } => run(first_descriptive_field_name),
        Command::Comment {
            first, // preserve field comment
            second,
        } => pair(first, second),
        Command::Comment { /* keep */ first, second } => pair(first, second),
    }
    let Pair { first, second } = pair;
    if let EnvSubcommand::Use { package_name } = command {
        run(package_name);
    }
    while let Pair { first, second } = next() {
        pair(first, second);
    }
...
fn width_limit() {
    match command {
        ExtremelyDescriptiveAndLongCommandNameThatConsumesMostOfTheAvailableLineWidth::Use {
            package_name,
        } => run(package_name),
    }
}
